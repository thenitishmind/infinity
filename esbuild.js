	...
	setup(build) {
		build.onEnd(() => {
			try {
				...
				const sourceFile = path.join(sourceDir, "tree-sitter.wasm")
				const targetFile = path.join(targetDir, "tree-sitter.wasm")
				
				if (fs.existsSync(sourceFile)) {
					fs.copyFileSync(sourceFile, targetFile)
				} else {
					console.warn(`Warning: tree-sitter.wasm not found at ${sourceFile}`)
				}
				...
				languages.forEach((lang) => {
					...
					const langSourceFile = path.join(languageWasmDir, filename)
					const langTargetFile = path.join(targetDir, filename)
					
					if (fs.existsSync(langSourceFile)) {
						fs.copyFileSync(langSourceFile, langTargetFile)
					} else {
						console.warn(`Warning: ${filename} not found at ${langSourceFile}`)
					}
				})
			} catch (error) {
				console.warn(`Warning: Failed to copy WASM files: ${error.message}`)
			}
		})
	},
}
...
const standaloneConfig = {
	...baseConfig,
	entryPoints: ["src/standalone/infinity-core.ts"],
	outfile: `${destDir}/infinity-core.js`,
