...
export class InfinityError extends Error {
	...
	/**
	 * Parses a stringified error into a InfinityError instance.
	 */
	static parse(errorStr?: string, modelId?: string): InfinityError | undefined {
		if (!errorStr || typeof errorStr !== "string") {
			return undefined
		}
		return InfinityError.transform(errorStr, modelId)
	}

	/**
	 * Transforms any object into a InfinityError instance.
	 * Always returns a InfinityError, even if the input is not a valid error object.
	 */
	static transform(error: any, modelId?: string, providerId?: string): InfinityError {
		try {
			return new InfinityError(JSON.parse(error), modelId, providerId)
		} catch {
			return new InfinityError(error, modelId, providerId)
		}
	}

	public isErrorType(type: ClineErrorType): boolean {
		return InfinityError.getErrorType(this) === type
	}
	 ...
	 */
	static getErrorType(err: InfinityError): ClineErrorType | undefined {
		...
		if (message?.includes(INFINITY_ACCOUNT_AUTH_ERROR_MESSAGE)) {
			return ClineErrorType.Auth
		}
