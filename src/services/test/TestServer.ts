...
export function createTestServer(webviewProvider?: WebviewProvider): http.Server {
	// Try to show the Infinity sidebar
	Logger.log("[createTestServer] Opening Infinity in sidebar...")
		vscode.commands.executeCommand("workbench.view.infinity-ActivityBar")

	// Then ensure the webview is focused/loaded
		vscode.commands.executeCommand("infinity.SidebarProvider.focus")
		...
		req.on("end", async () => {
			try {
				...
				if (!visibleWebview || !visibleWebview.controller) {
					res.writeHead(500)
					res.end(JSON.stringify({ error: "No active Infinity instance found" }))
					return
				}
				...
				try {
					...
					if (apiKey) {
						...
						const updatedConfig = {
							...apiConfiguration,
							apiProvider: "infinity" as ApiProvider,
						...
						// Update global state to use infinity provider
						await updateGlobalState(visibleWebview.controller.context, "planModeApiProvider", "infinity")
						await updateGlobalState(visibleWebview.controller.context, "actModeApiProvider", "infinity")
					...
					try {
						...
						try {
							...
							Logger.log(`Error getting saved Infinity messages: ${error}`)
						}
...
export function createMessageCatcher(webviewProvider: WebviewProvider): vscode.Disposable {
	Logger.log("Infinity message catcher registered")
	...
	return new vscode.Disposable(() => {
		...
		Logger.log("Infinity message catcher disposed")
	})
}
