...
export async function validateWorkspacePath(workspacePath: string): Promise<void> {
	...
	try {
		const testFile = path.join(workspacePath, ".infinity_write_test")
...
export async function initializeGitRepository(workspacePath: string): Promise<boolean> {
	...
	try {
		...
		await execa("git", ["config", "user.name", "Infinity Evaluation"], { cwd: workspacePath })
		await execa("git", ["config", "user.email", "infinity@example.com"], { cwd: workspacePath })
