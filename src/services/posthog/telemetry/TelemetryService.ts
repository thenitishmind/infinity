...
/**
 * TelemetryService handles telemetry event tracking for the Infinity extension
...
class TelemetryService {
	...
	public async updateTelemetryState(didUserOptIn: boolean): Promise<void> {
		...
		if (globalTelemetryEnabled) {
			...
			// Only show warning if user has opted in to Infinity telemetry but VS Code telemetry is disabled
			if (didUserOptIn) {
				void vscode.window
					.showWarningMessage(
						"Anonymous Infinity error and usage reporting is enabled, but VSCode telemetry is disabled. To enable error and usage reporting for this extension, enable VSCode telemetry in settings.",
						"Open Settings",
					)
					.then((selection) => {
	...
	/**
	 * Records when infinity calls the task completion_result tool signifying that infinity is done with the task
