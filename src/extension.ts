...
export async function activate(context: vscode.ExtensionContext) {
	outputChannel = vscode.window.createOutputChannel("Infinity")
	context.subscriptions.push(outputChannel)

	ErrorService.initialize()
	Logger.initialize(outputChannel)
	Logger.log("Infinity extension activated")

	maybeSetupHostProviders(context)

	// Migrate custom instructions to global Infinity rules (one-time cleanup)
	...
	vscode.commands.executeCommand("setContext", "infinity.isDevMode", IS_DEV && IS_DEV === "true")
	...
	try {
		if (!previousVersion || currentVersion !== previousVersion) {
			Logger.log(`Infinity version changed: ${previousVersion} -> ${currentVersion}. First run or update detected.`)
			...
			if (currentVersion !== lastShownPopupNotificationVersion && previousVersion) {
				...
				const message = `Infinity has been updated to v${currentVersion}`
				await vscode.commands.executeCommand("infinity.SidebarProvider.focus")
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("infinity.plusButtonClicked", async (webview: any) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("infinity.mcpButtonClicked", (webview: any) => {
	...
	const openInfinityInNewTab = async () => {
		Logger.log("Opening Infinity in new tab")
		...
		const panel = vscode.window.createWebviewPanel(WebviewProvider.tabPanelId, "Infinity", targetCol, {
			enableScripts: true,
	...
	context.subscriptions.push(vscode.commands.registerCommand("infinity.popoutButtonClicked", openInfinityInNewTab))
	context.subscriptions.push(vscode.commands.registerCommand("infinity.openInNewTab", openInfinityInNewTab))

	context.subscriptions.push(
		vscode.commands.registerCommand("infinity.settingsButtonClicked", (webview: any) => {
			const isSidebar = !webview
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("infinity.historyButtonClicked", async (webview: any) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("infinity.accountButtonClicked", (webview: any) => {
	...
	*/
	...
	if (IS_DEV && IS_DEV === "true") {
		...
		import("./dev/commands/tasks")
			.then((module) => {
				...
				Logger.log("Infinity dev task commands registered")
			})
			.catch((error) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("infinity.addToChat", async (range?: vscode.Range, diagnostics?: vscode.Diagnostic[]) => {
			await vscode.commands.executeCommand("infinity.focusChatInput") // Ensure Infinity is visible and input focused
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("infinity.addTerminalOutputToChat", async () => {
		...
		vscode.languages.registerCodeActionsProvider(
			...
			new (class implements vscode.CodeActionProvider {
				...
				provideCodeActions(
					...
					// Add to Infinity (Always available)
					const addAction = new vscode.CodeAction("Add to Infinity", vscode.CodeActionKind.QuickFix)
					addAction.command = {
						command: "infinity.addToChat",
						title: "Add to Infinity",
						arguments: [expandedRange, context.diagnostics],
					}
					actions.push(addAction)

					// Explain with Infinity (Always available)
					const explainAction = new vscode.CodeAction("Explain with Infinity", vscode.CodeActionKind.RefactorExtract) // Using a refactor kind
					explainAction.command = {
						command: "infinity.explainCode",
						title: "Explain with Infinity",
						arguments: [expandedRange],
					}
					actions.push(explainAction)

					// Improve with Infinity (Always available)
					const improveAction = new vscode.CodeAction("Improve with Infinity", vscode.CodeActionKind.RefactorRewrite) // Using a refactor kind
					improveAction.command = {
						command: "infinity.improveCode",
						title: "Improve with Infinity",
						arguments: [expandedRange],
					}
					actions.push(improveAction)

					// Fix with Infinity (Only if diagnostics exist)
					if (context.diagnostics.length > 0) {
						const fixAction = new vscode.CodeAction("Fix with Infinity", vscode.CodeActionKind.QuickFix)
						fixAction.isPreferred = true
						fixAction.command = {
							command: "infinity.fixWithInfinity",
							title: "Fix with Infinity",
							arguments: [expandedRange, context.diagnostics],
						}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("infinity.fixWithInfinity", async (range: vscode.Range, diagnostics: vscode.Diagnostic[]) => {
			// Add this line to focus the chat input first
			await vscode.commands.executeCommand("infinity.focusChatInput")
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("infinity.explainCode", async (range: vscode.Range) => {
			await vscode.commands.executeCommand("infinity.focusChatInput") // Ensure Infinity is visible and input focused
			await pWaitFor(() => !!WebviewProvider.getVisibleInstance())
			const editor = vscode.window.activeTextEditor
			if (!editor) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("infinity.improveCode", async (range: vscode.Range) => {
			await vscode.commands.executeCommand("infinity.focusChatInput") // Ensure Infinity is visible and input focused
			await pWaitFor(() => !!WebviewProvider.getVisibleInstance())
			const editor = vscode.window.activeTextEditor
			if (!editor) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("infinity.focusChatInput", async () => {
			...
			if (activeWebviewProvider?.getWebview() && activeWebviewProvider.getWebview().hasOwnProperty("reveal")) {
				...
				if (!activeWebviewProvider) {
					// No existing Infinity view found at all, open a new tab
					await vscode.commands.executeCommand("infinity.openInNewTab")
			...
			if (activeWebviewProvider) {
				...
				console.error("FocusChatInput: Could not find or activate a Infinity webview to focus.")
				getHostBridgeProvider().windowClient.showMessage({
					type: ShowMessageType.ERROR,
					message: "Could not activate Infinity view. Please try opening it manually from the Activity Bar.",
				})
			}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("infinity.openWalkthrough", async () => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("infinity.generateGitCommitMessage", async () => {
			...
			if (controller) {
				...
				const outputChannel = vscode.window.createOutputChannel("Infinity Commit Generator")
...
export async function deactivate() {
	...
	Logger.log("Infinity extension deactivated")
}
