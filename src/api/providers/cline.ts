...
import { INFINITY_ACCOUNT_AUTH_ERROR_MESSAGE } from "@/shared/InfinityAccount"
...
export class ClineHandler implements ApiHandler {
	...
	private async ensureClient(): Promise<OpenAI> {
		...
		if (!clineAccountAuthToken) {
			throw new Error(INFINITY_ACCOUNT_AUTH_ERROR_MESSAGE)
		}
		if (!this.client) {
			try {
				this.client = new OpenAI({
					...
					defaultHeaders: {
						"HTTP-Referer": "https://infinity.bot",
						"X-Title": "Infinity",
						"X-Task-ID": this.options.taskId || "",
						"X-Infinity-Version": extensionVersion,
					},
				})
			} catch (error: any) {
				throw new Error(`Error creating Infinity client: ${error.message}`)
			}
		}
		...
		try {
			...
			for await (const chunk of stream) {
				...
				if ("error" in chunk) {
					...
					console.error(`Infinity API Error: ${error?.code} - ${error?.message}`)
					// Include metadata in the error message if available
					const metadataStr = error.metadata ? `\nMetadata: ${JSON.stringify(error.metadata, null, 2)}` : ""
					throw new Error(`Infinity API Error ${error.code}: ${error.message}${metadataStr}`)
				}
				...
				if ((choice?.finish_reason as string) === "error") {
					...
					if (choiceWithError.error) {
						...
						console.error(`Infinity Mid-Stream Error: ${error.code || error.type || "Unknown"} - ${error.message}`)
						throw new Error(`Infinity Mid-Stream Error: ${error.code || error.type || "Unknown"} - ${error.message}`)
					} else {
						throw new Error(
							"Infinity Mid-Stream Error: Stream terminated with error status but no error details provided",
						)
					}
				}
			...
			if (!didOutputUsage) {
				console.warn("Infinity API did not return usage chunk, fetching from generation endpoint")
	...
	async getApiStreamUsage(): Promise<ApiStreamUsageChunk | undefined> {
		if (this.lastGenerationId) {
			try {
				...
				// ignore if fails
				console.error("Error fetching infinity generation details:", error)
			}
		}
		return undefined
	}
