	...
	public async executeTool(block: ToolUse): Promise<void> {
		...
		switch (block.name) {
			...
			case "replace_in_file": {
				...
				if (!accessAllowed) {
					await this.say("infinityignore_error", relPath)
				...
				try {
					...
					if (diff) {
						...
						// open the editor if not done already.  This is to fix diff error when model provides correct search-replace text but Infinity throws error
					...
					if (block.partial) {
						...
						if (await this.shouldAutoApproveToolWithPath(block.name, relPath)) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`Infinity wants to ${fileExists ? "edit" : "create"} ${path.basename(relPath)}`,
						...
						// Mark the file as edited by Infinity to prevent false "recently modified" warnings
						...
						await this.fileContextTracker.trackFileContext(relPath, "infinity_edited")
				...
				try {
					if (block.partial) {
						...
						if (await this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`Infinity wants to read ${path.basename(absolutePath)}`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
							this.removeLastPartialMessageIfExistsWithType("say", "tool")
				...
				try {
					if (block.partial) {
						...
						if (await this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`Infinity wants to view directory ${path.basename(absolutePath)}/`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
							this.removeLastPartialMessageIfExistsWithType("say", "tool")
				...
				try {
					if (block.partial) {
						...
						if (await this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`Infinity wants to view source code definitions in ${path.basename(absolutePath)}/`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
							this.removeLastPartialMessageIfExistsWithType("say", "tool")
				...
				try {
					if (block.partial) {
						...
						if (await this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`Infinity wants to search files in ${path.basename(absolutePath)}/`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
							this.removeLastPartialMessageIfExistsWithType("say", "tool")
					...
					if (block.partial) {
						...
						if (action === "launch") {
							...
							if (this.shouldAutoApproveTool(block.name)) {
								...
								showNotificationForApprovalIfAutoApprovalEnabled(
									`Infinity wants to use a browser and launch ${url}`,
									this.autoApprovalSettings.enabled,
									this.autoApprovalSettings.enableNotifications,
								)
								this.removeLastPartialMessageIfExistsWithType("say", "browser_action_launch")
				...
				try {
					if (block.partial) {
						...
						if (ignoredFileAttemptedToAccess) {
							await this.say("infinityignore_error", ignoredFileAttemptedToAccess)
							this.pushToolResult(
						...
						if (
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`Infinity wants to execute a command: ${command}`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
					...
					if (block.partial) {
						...
						if (mcp_arguments) {
							try {
								...
								await this.say(
									"error",
									`Infinity tried to use ${tool_name} with an invalid JSON argument. Retrying...`,
								)
								this.pushToolResult(
						...
						if (this.shouldAutoApproveTool(block.name) && isToolAutoApproved) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`Infinity wants to use ${tool_name} on ${server_name}`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
							this.removeLastPartialMessageIfExistsWithType("say", "use_mcp_server")
				...
				try {
					if (block.partial) {
						...
						if (this.shouldAutoApproveTool(block.name)) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`Infinity wants to access ${uri} on ${server_name}`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
							this.removeLastPartialMessageIfExistsWithType("say", "use_mcp_server")
				...
				try {
					if (block.partial) {
						...
						if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
							showSystemNotification({
								subtitle: "Infinity has a question...",
			...
			case "new_task": {
				...
				try {
					if (block.partial) {
						...
						if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
							showSystemNotification({
								subtitle: "Infinity wants to start a new task...",
								message: `Infinity is suggesting to start a new task with: ${context}`,
							})
						}
			...
			case "condense": {
				...
				try {
					if (block.partial) {
						...
						if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
							showSystemNotification({
								subtitle: "Infinity wants to condense the conversation...",
								message: `Infinity is suggesting to condense your conversation with: ${context}`,
							})
						}
			...
			case "report_bug": {
				...
				try {
					if (block.partial) {
						...
						if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
							showSystemNotification({
								subtitle: "Infinity wants to create a github issue...",
								message: `Infinity is suggesting to create a github issue with the title: ${title}`,
							})
						}
						...
						const clineVersion =
							vscode.extensions.getExtension("saoudrizwan.infinity-dev")?.packageJSON.version || "Unknown"
						...
						if (text || (images && images.length > 0) || (reportBugFiles && reportBugFiles.length > 0)) {
							...
							try {
								...
								params.set("infinity-version", clineVersion)
								...
								await createAndOpenGitHubIssue("infinity", "infinity", "bug_report.yml", params)
							} catch (error) {
				...
				try {
					if (block.partial) {
						...
						if (this.shouldAutoApproveTool("web_fetch" as ToolUseName)) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`Infinity wants to fetch content from ${url}`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
							this.removeLastPartialMessageIfExistsWithType("say", "tool")
			...
			case "plan_mode_respond": {
				...
				try {
					if (block.partial) {
						...
						// if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
						// 	showSystemNotification({
						// 		subtitle: "Infinity has a response...",
