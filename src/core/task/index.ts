...
import { ClineHandler } from "@api/providers/infinity"
...
import { ClineErrorType } from "@/services/error/InfinityError"
...
} from "@core/context/instructions/user-instructions/infinity-rules"
import {
	getLocalCursorRules,
...
import { convertClineMessageToProto } from "@shared/proto-conversions/infinity-message"
...
export class Task {
	...
	async ask(
		...
		// If this Infinity instance was aborted by the provider, then the only thing keeping us alive is a promise still running in the background, in which case we don't want to send its result to the webview as it is attached to a new instance of Infinity now. So we can safely ignore the result of any active promises, and this class will be deallocated. (Although we set Infinity = undefined in provider, that simply removes the reference to this instance, but the instance is still alive until this promise resolves or rejects.)
		if (this.taskState.abort) {
			throw new Error("Infinity instance aborted")
		}
		let askTs: number
	...
	async sayAndCreateMissingParamError(toolName: ToolUseName, paramName: string, relPath?: string) {
		await this.say(
			"error",
			`Infinity tried to use ${toolName}${
	...
	private async startTask(task?: string, images?: string[], files?: string[]): Promise<void> {
		...
		// if the extension process were killed, then on restart the clineMessages might not be empty, so we need to set it to [] when we create a new Infinity client (otherwise webview would show stale messages from previous session)
	...
	private async resumeTaskFromHistory() {
		...
		// Now present the infinity messages to the user and ask if they want to resume (NOTE: we ran into a bug before where the apiconversationhistory wouldn't be initialized when opening a old task, and it was because we were waiting for resume)
		...
		// need to make sure that the api conversation history can be resumed by the api, even if it goes out of sync with infinity messages
	...
	private async initiateTaskLoop(userContent: UserContent): Promise<void> {
		...
		while (!this.taskState.abort) {
			...
			//  The way this agentic loop works is that infinity will be given a task that he then calls tools to complete. unless there's an attempt_completion call, we keep responding back to him with his tool's responses until he either attempt_completion or does not use anymore tools. If he does not use anymore tools, we ask him to consider if he's completed the task and then call attempt_completion, otherwise proceed with completing the task.
			// There is a MAX_REQUESTS_PER_TASK limit to prevent infinite requests, but Infinity is prompted to finish the task as efficiently as he can.
			...
			if (didEndLoop) {
				...
				// this.say(
				// 	"tool",
				// 	"Infinity responded with only text blocks but has not called attempt_completion yet. Forcing him to continue with task..."
				// )
				nextUserContent = [
					{
						type: "text",
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		// infinity browser tool uses image recognition for navigation (requires model image support).
	...
	async recursivelyMakeClineRequests(userContent: UserContent, includeFileDetails: boolean = false): Promise<boolean> {
		...
		if (this.taskState.consecutiveMistakeCount >= 3) {
			if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
				showSystemNotification({
					subtitle: "Error",
					message: "Infinity is having trouble. Would you like to continue the task?",
				})
			}
			const { response, text, images, files } = await this.ask(
				...
				this.api.getModel().id.includes("claude")
					...
					: "Infinity uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 4 Sonnet for its advanced agentic coding capabilities.",
			)
			if (response === "messageResponse") {
		...
		if (
			...
			if (this.autoApprovalSettings.enableNotifications) {
				showSystemNotification({
					...
					message: `Infinity has auto-approved ${this.autoApprovalSettings.maxRequests.toString()} API requests.`,
				})
			}
			const { response, text, images, files } = await this.ask(
				"auto_approval_max_req_reached",
				`Infinity has auto-approved ${this.autoApprovalSettings.maxRequests.toString()} API requests. Would you like to reset the count and proceed with the task?`,
			)
			...
			try {
				...
				checkpointsWarningTimer = setTimeout(async () => {
					if (!checkpointsWarningShown) {
						...
						this.taskState.checkpointTrackerErrorMessage =
							"Checkpoints are taking longer than expected to initialize. Working in a large repository? Consider re-opening Infinity in a project that uses git, or disabling checkpoints."
						await this.postStateToWebview()
					}
				}, 7_000)
				...
				this.checkpointTracker = await pTimeout(
					CheckpointTracker.create(this.taskId, this.context.globalStorageUri.fsPath, this.enableCheckpoints),
					{
						milliseconds: 15_000,
						message:
							"Checkpoints taking too long to initialize. Consider re-opening Infinity in a project that uses git, or disabling checkpoints.",
					},
				)
				...
				if (errorMessage.includes("Checkpoints taking too long to initialize")) {
					this.taskState.checkpointTrackerErrorMessage =
						"Checkpoints initialization timed out. Consider re-opening Infinity in a project that uses git, or disabling checkpoints."
					await this.postStateToWebview()
				} else {
		...
		const [parsedUserContent, environmentDetails, infinityrulesError] = await this.loadContext(userContent, includeFileDetails)

		// error handling if the user uses the /newrule command & their .infinityrules is a file, for file read operations didnt work properly
		if (infinityrulesError === true) {
			await this.say(
				"error",
				"Issue with processing the /newrule command. Double check that, if '.infinityrules' already exists, it's a directory and not a file. Otherwise there was an issue referencing this file/directory.",
			)
		}
		...
		try {
			...
			try {
				for await (const chunk of stream) {
					...
					if (this.taskState.abort) {
						...
						if (!this.taskState.abandoned) {
							// only need to gracefully abort if this instance isn't abandoned (sometimes openrouter stream hangs, in which case this would affect future instances of infinity)
				...
				// abandoned happens when extension is no longer waiting for the infinity instance to finish aborting (error is thrown here when any function in the for loop throws due to this.abort)
				if (!this.taskState.abandoned) {
			...
			// OpenRouter/Infinity may not return token usage as part of the stream (since it may abort early), so we fetch after the stream is finished
	...
	async loadContext(userContent: UserContent, includeFileDetails: boolean = false): Promise<[UserContent, string, boolean]> {
		// Track if we need to check infinityrulesFile
		...
		// After processing content, check infinityrulesData if needed
		let infinityrulesError = false
		if (needsClinerulesFileCheck) {
			infinityrulesError = await ensureLocalClineDirExists(this.cwd, GlobalFileNames.clineRules)
		}

		// Return all results
		return [processedUserContent, environmentDetails, infinityrulesError]
	}

	async getEnvironmentDetails(includeFileDetails: boolean = false) {
		let details = ""

		// It could be useful for infinity to know if the user went from one or no file to another between messages, so we always include this context
		...
		const diagnostics = await this.diagnosticsMonitor.getCurrentDiagnostics(this.didEditFile || terminalWasBusy) // if infinity ran a command (ie npm install) or edited the workspace then wait a bit for updated diagnostics
