 ...
 * Designed to be instantiated once in Infinity.ts and passed to file manipulation services.
 * Uses the 'ignore' library to support standard .gitignore syntax in .infinityignore files.
 */
export class ClineIgnoreController {
	...
	constructor(cwd: string) {
		...
		// Set up file watcher for .infinityignore
	...
	/**
	 * Set up the file watcher for .infinityignore changes
	 */
	private setupFileWatcher(): void {
		const infinityignorePattern = new vscode.RelativePattern(this.cwd, ".infinityignore")
		const fileWatcher = vscode.workspace.createFileSystemWatcher(infinityignorePattern)
	...
	/**
	 * Load custom patterns from .infinityignore if it exists.
	...
	private async loadClineIgnore(): Promise<void> {
		try {
			...
			this.ignoreInstance = ignore()
			const ignorePath = path.join(this.cwd, ".infinityignore")
			if (await fileExistsAtPath(ignorePath)) {
				const content = await fs.readFile(ignorePath, "utf8")
				this.clineIgnoreContent = content
				await this.processIgnoreContent(content)
				this.ignoreInstance.add(".infinityignore")
			} else {
				this.clineIgnoreContent = undefined
			}
		} catch (error) {
			// Should never happen: reading file failed even though it exists
			console.error("Unexpected error loading .infinityignore:", error)
		}
	}
	...
	private async readIncludedFile(includeLine: string): Promise<string | null> {
		...
		if (!(await fileExistsAtPath(resolvedIncludePath))) {
			console.debug(`[InfinityIgnore] Included file not found: ${resolvedIncludePath}`)
			return null
		}
	...
	validateAccess(filePath: string): boolean {
		// Always allow access if .infinityignore does not exist
	...
	validateCommand(command: string): string | undefined {
		// Always allow if no .infinityignore exists
		if (!this.clineIgnoreContent) {
			return undefined
		}
