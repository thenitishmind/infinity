...
export abstract class WebviewProvider {
	public static readonly sideBarId = "infinity.SidebarProvider" // used in package.json as the view's id. This value cannot be changed due to how vscode caches views based on their id, and updating the id would break existing instances of the extension.
	public static readonly tabPanelId = "infinity.TabPanelProvider"
	...
	public static getActiveInstance(): WebviewProvider | undefined {
		return Array.from(this.activeInstances).find((instance) => {
			if (
				instance.getWebview() &&
				instance.getWebview().viewType === "infinity.TabPanelProvider" &&
		...
		return /*html*/ `
			...
			<html lang="en">
				...
				<meta http-equiv="Content-Security-Policy" content="default-src 'none';
					connect-src https://*.posthog.com https://*.infinity.bot https://*.firebaseauth.com https://*.firebaseio.com https://*.googleapis.com https://*.firebase.com; 
		...
		try {
			...
			if (!process.env.IS_DEV) {
				getHostBridgeProvider().windowClient.showMessage({
					...
					message:
						"Infinity: Local webview dev server is not running, HMR will not work. Please run 'npm run dev:webview' before launching the extension to enable HMR. Using bundled assets.",
				})
			}
