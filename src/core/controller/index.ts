...
import { ClineRulesToggles } from "@shared/infinity-rules"
	...
	*/
	...
	async handleSignOut() {
		try {
			...
			getHostBridgeProvider().windowClient.showMessage({
				...
				message: "Successfully logged out of Infinity",
			})
		} catch (error) {
			getHostBridgeProvider().windowClient.showMessage({
				type: ShowMessageType.INFORMATION,
	...
	async cancelTask() {
		if (this.task) {
			...
			if (this.task) {
				// 'abandoned' will prevent this infinity instance from affecting future infinity instance gui. this may happen if its hanging on a streaming request
				this.task.taskState.abandoned = true
			}
			await this.initTask(undefined, undefined, undefined, historyItem) // clears task again, so we need to abortTask manually above
			// await this.postStateToWebview() // new Infinity instance will post state when it's ready. having this here sent an empty messages array to webview leading to virtuoso having to reload the entire list
		}
	}
	...
	async handleAuthCallback(customToken: string, provider: string | null = null) {
		try {
			...
			const clineProvider: ApiProvider = "infinity"
			...
			getHostBridgeProvider().windowClient.showMessage({
				...
				message: "Failed to log in to Infinity",
			})
	...
	private async fetchMcpMarketplaceFromApiRPC(silent: boolean = false): Promise<McpMarketplaceCatalog | undefined> {
		try {
			const response = await axios.get(`${clineEnvConfig.mcpBaseUrl}/marketplace`, {
				headers: {
					"Content-Type": "application/json",
					"User-Agent": "infinity-vscode-extension",
				},
			})
	...
	// 'Add to Infinity' context menu in editor and code action
	...
	// 'Add to Infinity' context menu in Terminal
	...
	// 'Fix with Infinity' in code actions
	...
	/*
	Now that we use retainContextWhenHidden, we don't have to store a cache of infinity messages in the user's state, but we could to reduce memory footprint in long conversations.

	- We have to be careful of what state is shared between ClineProvider instances since there could be multiple instances of the extension running at once. For example when we cached infinity messages using the same key, two instances of the extension could end up using the same key and overwriting each other's messages.
