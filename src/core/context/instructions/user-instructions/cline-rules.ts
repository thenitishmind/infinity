	...
	if (await fileExistsAtPath(globalClineRulesFilePath)) {
		if (await isDirectory(globalClineRulesFilePath)) {
			try {
				...
				console.error(`Failed to read .infinityrules directory at ${globalClineRulesFilePath}`)
			}
		} else {
...
export const getLocalClineRules = async (cwd: string, toggles: ClineRulesToggles) => {
	...
	if (await fileExistsAtPath(clineRulesFilePath)) {
		if (await isDirectory(clineRulesFilePath)) {
			try {
				const rulesFilePaths = await readDirectory(clineRulesFilePath, [[".infinityrules", "workflows"]])
				...
				console.error(`Failed to read .infinityrules directory at ${clineRulesFilePath}`)
			}
		} else {
			try {
				...
				console.error(`Failed to read .infinityrules file at ${clineRulesFilePath}`)
			}
		}
	}
