...
export const DIFF_VIEW_URI_SCHEME = "infinity-diff"
...
export class VscodeDiffViewProvider extends DiffViewProvider {
	...
	override async openDiffEditor(): Promise<void> {
		...
		// get diagnostics before editing the file, we'll compare to diagnostics after editing to see if infinity needs to fix anything
		...
		if (diffTab && diffTab.input instanceof vscode.TabInputTextDiff) {
			...
			this.activeDiffEditor = await new Promise<vscode.TextEditor>((resolve, reject) => {
				...
				vscode.commands.executeCommand(
					...
					uri,
					`${fileName}: ${fileExists ? "Original ↔ Infinity's Changes" : "New File"} (Editable)`,
					{
						preserveFocus: true,
					},
				)
	...
	protected async closeDiffView(): Promise<void> {
		// Close all the infinity diff views.
		const tabs = vscode.window.tabGroups.all
			.flatMap((tg) => tg.tabs)
