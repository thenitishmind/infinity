...
async function generateHandlersAndExports() {
	...
	for (const [name, def] of Object.entries(protobusServices)) {
		...
		handlerSetup.push(`    server.addService(infinity.${name}Service, {`)
		for (const [rpcName, rpc] of Object.entries(def.service)) {
			imports.push(`import { ${rpcName} } from "@core/controller/${dir}/${rpcName}"`)
			const requestType = "infinity." + rpc.requestType.type.name
			...
			if (rpc.responseStream) {
				...
				const responseType = "infinity." + rpc.responseType.type.name
...
import { infinity } from "@generated/grpc-js"
