...
const getClineVersion = () => {
	try {
		const extensions = execSync("code --list-extensions --show-versions").toString()
				const infinityMatch = extensions.match(/infinity-dev@(\d+\.\d+\.\d+)/)
...
const createIssueUrl = (systemInfo, issueTitle) => {
	return (
		`https://github.com/infinity/infinity/issues/new?template=bug_report.yml` +
		`&title=${issueTitle}` +
		`&operating-system=${systemInfo.os}` +
		`&infinity-version=${systemInfo.clineVersion}` +
		`&system-info=${
