...
description: "Remote browser support allows Infinity to utilize a remote Chrome instance, leveraging authentication tokens and session cookies relevant to certain web development test cases."
icon: globe-pointer
---

The Remote Browser feature in Infinity allows the AI assistant to interact with web content directly through a controlled browser instance. This enables several powerful capabilities:
...
## Remote Browser in Infinity

### What is Remote Browser?

Remote Browser allows Infinity to view and interact with websites directly. This feature enables Infinity to:
...
You can ask Infinity to use the browser with simple instructions:
...
Infinity can only use one browser at a time. If you want to visit a different website, you can either:

-   Ask Infinity to navigate to a new URL within the same browser session
-   Ask Infinity to close the current browser and open a new one

#### Browser Must Be Closed Before Using Other Tools

If you want Infinity to edit files or run commands after using the browser, you must first ask it to close the browser:
...
The browser has a fixed viewport size (900x600 pixels by default), similar to a small laptop screen. Infinity will share screenshots after each action so you can see exactly what it sees.

#### Console Logs

Infinity captures browser console logs, which can be helpful for debugging web applications. These logs are included with each screenshot.

### Common Use Cases

-   **Web Development**: Test your websites and web applications
-   **UI/UX Review**: Get feedback on website design and usability
-   **Content Research**: Have Infinity browse websites to gather information
...
-   **If the browser seems stuck**: Ask Infinity to close the browser and try again
...
#### Configure Infinity in VS Code:

1. Open VS Code settings
2. Search for "Infinity: Chrome Executable Path"
3. Set the value to the path of your Chrome executable (e.g., `C:\Program Files\Google\Chrome\Application\chrome.exe`)

Infinity should now be able to use the Remote Browser feature from within WSL.
