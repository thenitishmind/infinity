...
description: "Learn how to configure AWS Bedrock to use AWS Profiles for authentication with Infinity, focusing on SSO/Federated roles for secure access."
---

### Overview

Infinity offers the option of utilizing AWS credentials or AWS profiles to access AWS Bedrock services. SSO/Federated roles are suggested over Legacy IAM configuration; this guide describes how to configure your environment so that Infinity uses SSO roles for authentication.
...
3. Continue configuring your profile using [the `aws configure sso` CLI wizard](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-sso.html#cli-configure-sso-configure)
    ...
    - Note which profile name you attach to your AWS account, this is needed to configure Infinity in the following steps

4. If you haven't already done so, install VSCode and the Infinity extension. Consult the [Getting Started](/getting-started) page for guidance.

5. Open the Infinity extension, then click on the settings button ⚙️ to select your API Provider.
...
<Frame>
	<img
		src="https://storage.googleapis.com/infinity_public_images/docs/assets/infinity-aws-setup-markup%20(1).png"
		alt="AWS Bedrock configuration in Infinity settings showing profile authentication setup"
	/>
</Frame>
