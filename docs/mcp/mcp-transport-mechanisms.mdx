...
description: "Learn about the two primary transport mechanisms for communication between Infinity and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases."
---

Model Context Protocol (MCP) supports two primary transport mechanisms for communication between Infinity and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (Infinity) spawns an MCP server as a child process
...
-   **Locality**: Runs on the same machine as Infinity
...
-   Single-client scenarios (one Infinity instance per server)
...
1. The client (Infinity) connects to the server's SSE endpoint via HTTP GET request
...
-   **Remote Access**: Can be hosted on a different machine from your Infinity instance
...
STDIO servers run locally on the same machine as Infinity, which has several important implications:
...
-   **Execution**: Starts and stops with Infinity (child process lifecycle)
...
-   Start when needed by Infinity
-   Not require network configuration
-   Need to be installed alongside Infinity or via a package manager
...
## Configuring Transports in Infinity

For detailed information on configuring STDIO and SSE transports in Infinity, including examples, see [Configuring MCP Servers](/mcp/configuring-mcp-servers).
