---
title: "MCP Made Easy"
description: "Learn how to use the MCP Marketplace to discover, install, and configure MCP servers that enhance Infinity's capabilities with additional tools and resources."
---

## What's an MCP Server?

MCP servers are specialized extensions that enhance Infinity's capabilities. They enable Infinity to perform additional tasks like fetching web pages, processing images, accessing APIs, and much more.
...
-   In Infinity, click the "Extensions" button (square icon) in the top toolbar
...
-   If the server requires an API key (most do), Infinity will guide you through:
...
-   Infinity will show confirmation when installation is complete
-   Check the server status in Infinity's MCP settings UI

### 5. Using Your New Server

-   After successful installation, Infinity will automatically integrate the server's capabilities
-   You'll see new tools and resources available in Infinity's system prompt
-   Simply ask Infinity to use the capabilities of your new server
-   Example: "Search the web for recent React updates using Perplexity"

**Corporate Users:** If you're using Infinity in a corporate environment, ensure you have permission to install third-party MCP servers according to your organization's security policies.
...
-   The server code is cloned/installed to `/Users/<username>/Documents/Infinity/MCP/`
...
-   This file is located at: `/Users/<username>/Library/Application Support/Code/User/globalStorage/saoudrizwan.infinity-dev/settings/infinity_mcp_settings.json`
-   Environment variables (like API keys) are securely stored
-   The server path is registered

### 3. Server Launch

-   Infinity detects the configuration change
-   Infinity launches your server as a separate process
-   Communication is established via stdio or HTTP

### 4. Integration with Infinity

-   Your server's capabilities are added to Infinity's system prompt
-   Tools become available via `use_mcp_tool` commands
-   Resources become available via `access_mcp_resource` commands
-   Infinity can now use these capabilities when prompted by the user
...
1. Open the MCP settings file: `/Users/<username>/Library/Application Support/Code/User/globalStorage/saoudrizwan.infinity-dev/settings/infinity_mcp_settings.json`
...
-   Try removing and reinstalling the server (remove from both the `infinity_mcp_settings.json` file and the `/Users/<username>/Documents/Infinity/MCP/` directory)
-   Use a terminal and run the command with its arguments directly. This will allow you to see the same errors that Infinity is seeing

## MCP Server Rules

Infinity is already aware of your active MCP servers and what they are for, but when you have a lot of MCP servers enabled, it can be useful to define when to use each server.

Utilize a `.infinityrules` file or custom instructions to support intelligent MCP server activation through keyword-based triggers, making Infinity's tool selection more intuitive and context-aware.

### How MCP Rules Work

MCP Rules group your connected MCP servers into functional categories and define trigger keywords that activate them automatically when detected in your conversations with Infinity.
...
-   **Contextual Tool Selection**: Infinity selects appropriate tools based on conversation context
...
When you write "Can you scrape this website?", Infinity detects "scrape" and "website" as triggers, automatically selecting web-related MCP servers.
...
Add this to your `.infinityrules` file or to your custom instructions to make Infinity's MCP server selection more intuitive and context-aware.
