...
description: "This protocol is designed to streamline the development process of building MCP servers with Infinity."
---

> 🚀 **Build and share your MCP servers with the world.** Once you've created a great MCP server, submit it to the [Infinity MCP Marketplace](https://github.com/infinity/mcp-marketplace) to make it discoverable and one-click installable by thousands of developers.

## What Are MCP Servers?

Model Context Protocol (MCP) servers extend AI assistants like Infinity by giving them the ability to:
...
The heart of effective MCP server development is following a structured protocol. This protocol is implemented through a `.infinityrules` file that lives at the **root** of your MCP working directory (/Users/your-name/Documents/Infinity/MCP).

### Using `.infinityrules` Files

A `.infinityrules` file is a special configuration that Infinity reads automatically when working in the directory where it's placed. These files:

-   Configure Infinity's behavior and enforce best practices
-   Switch Infinity into a specialized MCP development mode
...
Here's the complete MCP Server Development Protocol that should be placed in your `.infinityrules` file:
...
When this `.infinityrules` file is present in your working directory, Infinity will:
...
### 1. Create a `.infinityrules` file (🚨 IMPORTANT)

First, add a `.infinityrules` file to the root of your MCP working directory using the protocol above. This file configures Infinity to use the MCP development protocol when working in this folder.

### 2. Start a Chat with a Clear Description

Begin your Infinity chat by clearly describing what you want to build. Be specific about:
...
Infinity will automatically start in PLAN MODE, guiding you through the planning process:
...
One of the most effective ways to help Infinity build your MCP server is to share official API documentation right at the start:
...
Providing comprehensive API details (endpoints, authentication, data structures) significantly improves Infinity's ability to implement an effective MCP server.

## Understanding the Two Modes

### PLAN MODE

In this collaborative phase, you work with Infinity to design your MCP server:
...
Once planning is complete, Infinity helps implement the server:
...
<Frame>
	<img
		src="https://storage.googleapis.com/infinity_public_images/docs/assets/planning-phase.gif"
...
<Frame>
	<img
		src="https://storage.googleapis.com/infinity_public_images/docs/assets/building-mcp-plugin.gif"
...
Resources let your MCP servers expose data to Infinity without executing code. They're perfect for providing context like files, API responses, or database records that Infinity can reference during conversations.
...
Resources make your MCP servers more context-aware, allowing Infinity to access specific information without requiring you to copy/paste. For more information, refer to the [official documentation](https://modelcontextprotocol.io/docs/concepts/resources).
