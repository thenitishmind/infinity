...
description: "The Model Context Protocol (MCP) allows Infinity to communicate with external servers that provide additional tools and resources to extend its capabilities. This guide explains how to add and connect to remote MCP servers through the MCP Servers interface."
---
...
To access the MCP Servers interface in Infinity:

1. Click on the Infinity icon in the VSCode sidebar
2. Open the menu (⋮) in the top right corner of the Infinity panel
...
4. Infinity will attempt to connect to the server and display the connection status
...
> **Warning**: The following third-party marketplaces are listed for informational purposes only. Infinity does not endorse, verify, or take responsibility for any servers listed on these marketplaces. These servers are cloud-hosted services that process your requests and may have access to data you share with them. Always review privacy policies and terms of use before connecting to third-party services.
...
2. **Request Timeout**:
    - Set how long Infinity should wait for server responses
...
-   **Enabled**: Infinity can use the server's tools and resources
...
For advanced users, Infinity stores MCP server configurations in a JSON file that can be modified:
...
Once connected, Infinity can use the tools and resources provided by the MCP server. When Infinity suggests using an MCP tool:

1. A tool approval prompt will appear (unless auto-approved)
2. Review the tool details and parameters before approving
3. The tool will execute and return results to Infinity
