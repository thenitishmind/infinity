...
description: "Complete guide to resolving terminal integration issues in Infinity"
---

This guide helps you resolve terminal integration issues in Infinity. Terminal integration is crucial for Infinity to execute commands and read their output, enabling it to understand errors, test results, and command responses.

<Tip>
	If you're experiencing terminal issues, try switching to a simpler shell like `bash` in the Infinity settings, under "Terminal Settings"
...
```mermaid
graph TD
    A[Terminal Issue] --> B{Can Infinity execute commands?}
    B -->|No| C[Shell Integration Unavailable]
    B -->|Yes| D{Can Infinity see the output?}
...
-   Commands execute but Infinity can't read output
-   Terminal works fine manually but not with Infinity
...
-   Infinity states in chat: "[Command is running but producing no output]"
-   Commands complete but Infinity doesn't see results
...
-   **Increase Shell Integration Timeout**
    ...
    1. Within Infinity, left-click the **Settings** button in the top right-hand corner of the chat window
...
-   Recommend AI to use file output instead
    1. Tell Infinity in chat or Infinity rules, to use `command > output.txt` before reading the file/s

<Tip>
	This family of issues is only partially solved in the latest Infinity versions, so if you still face this, create a GitHub issue
...
-   Docker builds never complete in Infinity
-   Progress bars consume thousands of tokens
-   The Infinity button "Proceed while running" doesn't work properly in chat

<Tip>
	This family of issues has been solved in latest Infinity versions but if you still face any issues, then create a GitHub issue
	for this.
</Tip>

## Terminal Settings Explained

Access these in Infinity by clicking the settings icon, and navigating to the "Terminal Settings" section:

### Default Terminal Profile

-   **What it does**: Selects which shell Infinity uses for commands
...
-   **What it does**: How long Infinity waits for the terminal to be ready
-   **Default**: 4 seconds
-   **When to increase**:
...
-   **What it does**: Limits how many lines Infinity reads from terminal output
-   **Default**: 500 lines
-   **When to adjust**:
...
3. Select "WSL Bash" as terminal profile in Infinity

#### Path Issues

Windows path problems:

1. Use forward slashes in Infinity: `C:/Users/...`
...
1. Install Infinity on the remote machine, not locally
...
1. Install Infinity in the container
...
3. Choose "Trace"
4. Check Output panel → "Infinity" for terminal logs
...
### Why does Infinity create so many terminals?

When shell integration fails, Infinity can't reuse terminals safely (they might be running long processes). Enable shell integration or adjust the terminal reuse setting.

### Can I use my custom shell (nushell, xonsh, etc.)?

Infinity officially supports bash, zsh, fish, and PowerShell. Custom shells may work but aren't guaranteed. Use bash as a fallback.
...
Working integration shows command output in Infinity's chat. Failed integration shows "Shell Integration Unavailable" or "[Command is running but producing no output]".
...
2. **Report the Issue**:
    - Use `/reportbug` in Infinity github issues
