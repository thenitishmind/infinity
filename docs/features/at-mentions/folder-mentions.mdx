...
Folder mentions let you bring entire directories into your conversation with Infinity. Just type `@/` followed by a folder path ending with a slash, and Infinity gets access to the folder structure and its contents.

When you type `@/` in chat, Infinity shows your workspace files and folders. Navigate to the folder you want, make sure to include the trailing slash, and Infinity will see the folder's structure and contents.
...
Infinity can then see all the files in the auth directory, their contents, and how they relate to each other. This gives it the full context to explain complex interactions between multiple files.

Folder mentions are also perfect for getting help with project organization. When I'm unsure if my project structure makes sense, I'll ask Infinity to review it:
...
Next time you're working with multiple related files, try using folder mentions instead of referencing each file individually. You'll get more comprehensive help because Infinity can see the bigger picture of how everything fits together.

## How It Works Under the Hood

When you use a folder mention in your message, here's what happens behind the scenes:

1. When you send your message, Infinity detects the `@/path/to/folder/` pattern (with trailing slash) in your text
