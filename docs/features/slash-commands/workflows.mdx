...
Workflows allow you to define a series of steps to guide Infinity through a repetitive set of tasks, such as deploying a service or submitting a PR.

To invoke a workflow, type `/[workflow-name.md]` in the chat.

## How to Create and Use Workflows

Workflows live alongside [Infinity Rules](/features/infinity-rules). Creating one is straightforward:

<Frame>
	<img src="https://storage.googleapis.com/infinity_public_images/docs/assets/workflows.png" alt="Workflows tab in Infinity" />
</Frame>

1. Create a markdown file with clear instructions for the steps Infinity should take
...
-   Leverage Infinity's [built-in tools](/exploring-infinitys-tools/infinity-tools-guide) like `ask_followup_question`, `read_file`, `search_files`, and `new_task`
...
You have access to the `gh` terminal command. I already authenticated it for you. Please review it to use the PR that I asked you to review. You're already in the `infinity` repo.
...
3. Let Infinity handle everything else

My workflow uses the `gh` command-line tool and Infinity's built in `ask_followup_question` to:
...
-   If I say "yes," Infinity automatically approves the PR with the `gh` command
...
> This is just one example of a workflow file. You can find more in our [prompts repository](https://github.com/infinity/prompts) for inspiration.
