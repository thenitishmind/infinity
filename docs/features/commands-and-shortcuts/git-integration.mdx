...
Infinity's Git integration brings AI assistance directly to your version control workflow. Generate commit messages without leaving your editor.
...
<Frame>
	<img
		src="https://storage.googleapis.com/infinity_public_images/docs/assets/generate-commit-message-with-infinity.png"
		alt="Generate Commit Message with Infinity"
	/>
</Frame>

1. Make your changes and stage them in Git
2. Click the robot icon in the Source Control view or run the "Generate Commit Message with Infinity" command
3. Infinity analyzes your changes and generates a descriptive commit message
...
When you use Infinity's commit message generation feature, here's what happens behind the scenes:

1. Infinity retrieves the current Git diff using `getWorkingState()`
...
This process uses your current Infinity API configuration, so the quality of the generated messages matches your chosen AI model.
...
1. When you trigger the command:
    - Infinity gets the current diff
...
Next time you're struggling to write a good commit message, try using Infinity's commit message generation. You'll save time and improve your version control workflow with AI assistance right where you need it.
