...
import { InfinityError, ClineErrorType } from "../../../../src/services/error/InfinityError"
...
interface ErrorRowProps {
	...
	errorType: "error" | "mistake_limit_reached" | "auto_approval_max_req_reached" | "diff_error" | "infinityignore_error"
...
const ErrorRow = memo(({ message, errorType, apiRequestFailedMessage, apiReqStreamingFailedMessage }: ErrorRowProps) => {
		...
		switch (errorType) {
			...
			case "auto_approval_max_req_reached":
				...
				if (apiRequestFailedMessage || apiReqStreamingFailedMessage) {
					const clineError = InfinityError.parse(apiRequestFailedMessage || apiReqStreamingFailedMessage)
					const clineErrorMessage = clineError?.message
					const requestId = clineError?._error?.request_id
					const isClineProvider = clineError?.providerId === "infinity"
						...
						<p className="m-0 whitespace-pre-wrap text-[var(--vscode-errorForeground)] wrap-anywhere">
							...
							{clineErrorMessage?.toLowerCase()?.includes("powershell") && (
								<>
									...
									<a
										href="https://github.com/infinity/infinity/wiki/TroubleShooting-%E2%80%90-%22PowerShell-is-not-recognized-as-an-internal-or-external-command%22"
							...
							{clineError?.isErrorType(ClineErrorType.Auth) && (
								<>
									<br />
									<br />
									{/* The user is signed in or not using infinity provider */}
			...
			case "infinityignore_error":
				return (
					<div className="flex flex-col p-2 rounded text-xs bg-[var(--vscode-textBlockQuote-background)] text-[var(--vscode-foreground)] opacity-80">
						<div>
							Infinity tried to access <code>{message.text}</code> which is blocked by the <code>.infinityignore</code>
							file.
						</div>
					</div>
				)
	...
	// For diff_error and infinityignore_error, we don't show the header separately
	if (errorType === "diff_error" || errorType === "infinityignore_error") {
