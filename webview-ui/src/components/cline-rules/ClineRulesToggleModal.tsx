...
const ClineRulesToggleModal: React.FC = () => {
	...
	const toggleRule = (isGlobal: boolean, rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleClineRule(
			...
			.catch((error) => {
				console.error("Error toggling Infinity rule:", error)
			})
	}
	...
	return (
		<div ref={modalRef}>
			<div ref={buttonRef} className="inline-flex min-w-0 max-w-full">
				<Tooltip tipText="Manage Infinity Rules & Workflows" visible={isVisible ? false : undefined}>
					<VSCodeButton
						appearance="icon"
						aria-label="Infinity Rules"
			...
			{isVisible && (
				<div
					...
					<div className="text-xs text-[var(--vscode-descriptionForeground)] mb-4">
						{currentView === "rules" ? (
							<p>
								Rules allow you to provide Infinity with system-level guidance. Think of them as a persistent way to
								include context and preferences for your projects or globally for every conversation.{" "}
								<VSCodeLink
									href="https://docs.infinity.bot/features/infinity-rules"
							...
							<p>
								Workflows allow you to define a series of steps to guide Infinity through a repetitive set of tasks,
								...
								in the chat.{" "}
								<VSCodeLink
									href="https://docs.infinity.bot/features/slash-commands/workflows"
									style={{ display: "inline" }}
									className="text-xs">
									Docs
								</VSCodeLink>
